...
def split_tokenize(corpus: List[str]) -> Tuple[List[List[str]], Dict[str, int]]:
    words_freq = {}
    tokens = []
...
def simple_tokenize(corpus: List[str]) -> Tuple[List[List[str]], Dict[str, int]]:
    words_freq = {}
    tokens = []
    for sentence in corpus:
        if sentence:
...
def zipf_law(corpus: List[str], ax):
    _, words_freq = simple_tokenize(corpus=corpus)
...
def bag_of_words(corpus: List[str], vocab_size: int=None, tokenize_function: Callable=simple_tokenize) -> pd.DataFrame:
    tokenized_corpus, words_freq = tokenize_function(corpus=corpus)
...
def bag_of_ngrams(corpus: List[str], n: int=2, vocab_size: int=None, tokenize_function: Callable=simple_tokenize) -> pd.DataFrame:
    tokenized_corpus, _= tokenize_function(corpus=corpus)
...
def tfidf(corpus: List[str], vocab_size: int=None, tokenize_function: Callable=simple_tokenize) -> pd.DataFrame:
    tokenized_corpus, words_freq = tokenize_function(corpus=corpus)
