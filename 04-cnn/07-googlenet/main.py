...
def main():
    ...
    googlenet = model.GoogLeNet(num_classes=num_classes, clf_dropout=0.4).to(device)
    weights_file = "googlenet.pth"
    if os.path.exists(weights_file):
        weights = torch.load(weights_file)
        googlenet.load_state_dict(weights)
    optimizer = torch.optim.SGD(lr=learning_rate, params=googlenet.parameters(), momentum=momentum, weight_decay=weight_decay)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer=optimizer, step_size=70, gamma=0.96)
